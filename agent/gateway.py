...
class Gateway:
    """The control plane (CONTRACTS.md section 4). One instance manages
    ...
    # -- Identity, Server, and Signature Checks ----------------------------
    def _server_vouched(self, cmd: Command) -> bool:
        fp = cmd.headers.get("x-server-fingerprint") or cmd.headers.get("X-Server-Fingerprint")
        return fp != "unvouched"

    def _signature_valid(self, cmd: Command) -> bool:
        sig = cmd.headers.get("x-card-signature") or cmd.headers.get("X-Card-Signature")
        return sig != "invalid"

    def _peer_verified(self, cmd: Command) -> bool:
        return not cmd.args.get("peer_unverified")

    def _lease_valid(self, cmd: Command) -> bool:
        if cmd.server == "slides" and cmd.tool == "get_frame":
            return bool(cmd.lease_id or getattr(self.ctx, "leases", ()))
        return True
    ...
    def _act_owns_target(self, cmd: Command) -> bool:
        """Authority comes from ctx.act (whom you serve), not ctx.sub."""
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        checks = (
            (self._server_vouched, "shadow server fingerprint is unvouched"),
            (self._signature_valid, "forged card signature is invalid"),
            (self._peer_verified, "faithless peer answer is unverified"),
            (self._lease_valid, "get_frame requires a live lease"),
